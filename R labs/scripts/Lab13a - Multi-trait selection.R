...
(b.sh <- MASS::ginv(as.matrix(P)) %*% as.matrix(G) %*% as.matrix(ecoW))
...
GEBVs$Selected <- GEBVs$SH > quantile(GEBVs$SH, ((100 - is) / 100))
# the number of pre-selected parents
sum(GEBVs$Selected)
head(GEBVs)
tail(GEBVs)

# check the estimated genetic gain
selected <- GEBVs$Selected

gain <- colMeans(GEBVs[selected, c("SDM","SRA")], na.rm = TRUE) -
        colMeans(GEBVs[, c("SDM","SRA")], na.rm = TRUE)
gain

# saving the file
write.csv(GEBVs, "GEBVs.csv", row.names = TRUE)
